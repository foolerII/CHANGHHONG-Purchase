...
window.rebateMockData = {
  ...
  /* 杂入单主数据：关联货款场景「校验杂入单存在性」调用外部系统接口的原型模拟，编号在此列表内视为真实存在 */
  jcOrders: ['JC20260901001', 'JC20260901002', 'JC20260901003'],
    ...
    quick: {
      ...
      sections: [
        ...
        },
        {
          ...
          columns: ['字段', '算法口径'],
          rows: [
            ['合同剩余返利金额', '预计返利金额 − 同合同行下 status = 1 或 2（有效 + 审批中）的关联金额合计（关联返利 / 关联到款 / 关联货款合并计算）；增补需求生效后，审批中金额也要扣减，见「关联方式扩展」组'],
        ...
        },
        {
          ...
          intro: '本组内容仅在顶栏「场景优化（增补）」开关打开时生效，关掉即为改动前的基线方案。范围：返利主流程不变，仅在「关联到款」下新增 4 类特殊场景，并新增「关联货款」大类；5 类新场景都需要走审批，原有关联返利、通用关联到款不受影响、不走审批。',
          columns: ['规则项', '内容'],
          rows: [
            ['结构', '「关联方式」三层：关联返利 / 关联到款 / 关联货款；「关联到款」下二级「关联类型」：通用关联（默认）/ 跨合同关联（同客户）/ 跨客户关联 / 厂商款补 / 其他款补'],
            ['数据来源', '4 类特殊到款场景数据仍来自到款系统，但改为跨合同共享的到款池（见「共享到款额度」）；关联货款数据来源为杂入单，编号存在性靠调用外部系统接口校验（原型用内置清单模拟），不做金额校验，允许同一编号重复关联、支持多张杂入单'],
            ...
            ['保存拆分两条路径', '编辑页点「保存」：关联返利 + 通用关联到款按原逻辑立即生效；4 类特殊到款 + 关联货款不立即生效，写为「审批中」状态并弹出分组提审页面，需要上传凭证、填写关联原因后才能点「提交审批」'],
